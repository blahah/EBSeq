EBMultiTest <-
	function(Data,NgVector=NULL,
			 Conditions,AllParti=NULL, 
			 sizeFactors, maxround, 
			 Pool=FALSE, NumBin=1000, 
			 ApproxVal=10^-10,
			 PoolLower=.25, 
			 PoolUpper=.75,
			 Print=TRUE, 
	...
	if(Pool==TRUE){
		...
		for(lv in 1:NumCond){
			DataforPoolSP.dvd[[lv]] <- matrix(DataList.unlist.dvd[,Conditions==levels(Conditions)[lv]],
											  nrow=dim(DataList.unlist)[1])	
	...
	for (lv in 1:nlevels(Conditions)){
		...
		# no matter sizeFactors is a vector or a matrix. 
		# Matrix should be columns are the normalization factors
	...
	#Initialize SigIn & ...
	AlphaIn <- if (Alpha) Alpha else 0.5
	BetaIn <- rep(if (Beta) Beta else 0.5, NoneZeroLength)
	...
	for (times in 1:maxround){
		...
		UpdateOutput <-
			suppressWarnings(
				LogNMulti(DataListIn.unlist, DataListSPIn,
						  EmpiricalRList.Good.mat, 
						  EmpiricalRList.Good.mat.SP,  
						  NumOfEachGroupIn, AlphaIn, 
						  BetaIn, PIn, NoneZeroLength, 
						  AllParti,Conditions,
						  skipOptim=fixHyper))
