LogNMulti <-
	function(Input, InputSP, EmpiricalR, 
			 EmpiricalRSP, NumOfEachGroup, 
			 AlphaIn, BetaIn,  PIn, 
			 NoneZeroLength, AllParti, 
			 Conditions, skipOptim=FALSE)
	{

    # For each gene (m rows of Input---m genes)
    # Save each gene's F0, F1 for further likelihood calculation. 
	FList <- sapply(1:nrow(AllParti), 
		function(i) {
			sapply(1:nlevels(as.factor(AllParti[i,])),
			 	function(j) {
			 		f0(do.call(cbind,InputSP[AllParti[i,]==j]),
			 		   AlphaIn, BetaIn, 
			 		   do.call(cbind,EmpiricalRSP[AllParti[i,]==j]), 
			 		   NumOfEachGroup, log=T)
				}
			)
		}, simplify=F) 
	...
	if (skipOptim) {
		message('skipping optimisation step')
		AlphaNew <- AlphaIn
		BetaNew <- BetaIn
		PNew <- PIn
	} else {
		Result <- optim(StartValue,
						LikefunMulti,
						InputPool=list(InputSPGood,
									   Input[zGood,],
									   ZEach[zGood,], 
									   NoneZeroLength,
									   EmpiricalR[zGood, ],
									   EmpiricalRSPGood, 
									   NumOfEachGroup.zGood, 
									   AllParti)
						)
